...
- Scope minimo adicional declarado: `https://www.googleapis.com/auth/calendar.events.freebusy`; el endpoint `freeBusy.query` no esta autorizado por `calendar.events`.
...
3. Calendario propietario: `Puragenda Review - Shared Business Calendar`, propiedad de `diego11dfc@gmail.com`, compartido con `contactopurocode@gmail.com` con permiso para modificar eventos.
4. Evento externo: `PRIVATE BUSY BLOCK - CONTENT MUST NOT BE READ`, 10 de agosto de 2026, 15:00-16:00, en el calendario compartido.
5. Usar datos ficticios para la cita: `Google Review Client`, `review-client@example.com`, servicio `Google OAuth Verification Test`.
...
4. Mostrar que la pantalla OAuth dice `Puragenda` y todos los permisos solicitados. Expandir los detalles de cada permiso y aceptar con la cuenta de demostracion.
