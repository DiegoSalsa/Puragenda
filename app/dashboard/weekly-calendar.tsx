...
import { X, Check, UserCheck, UserX, Loader2, Clock, Mail, User } from "lucide-react";
            ...
            <div className="grid grid-cols-[60px_repeat(7,1fr)] border-b border-white/[0.06]">
              ...
              {days.map((day) => {
                ...
                return (
                  <div
                    key={day.toISOString()}
                    className={`border-l border-white/[0.06] p-3 text-center ${isToday ? "bg-[#7C3AED]/5" : ""}`}
                  >
                    <p className="text-[10px] uppercase tracking-widest text-white/30">
                      {format(day, "EEE", { locale: es })}
                    </p>
                    <p className={`text-xl font-bold ${isToday ? "text-[#7C3AED]" : ""}`}>
                      {format(day, "d")}
                    </p>
                  </div>
                );
              ...
              <div key={hour} className="grid grid-cols-[60px_repeat(7,1fr)] border-b border-white/[0.03]">
                ...
                {days.map((day) => {
                  ...
                  return (
                    <div
                      key={day.toISOString()}
                      className={`border-l border-white/[0.06] min-h-[52px] p-1 ${isToday ? "bg-[#7C3AED]/[0.02]" : ""}`}
                    >
                      {apts.map((apt) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full rounded-lg border ${sc.bg} ${sc.border} p-1.5 text-left transition-all hover:scale-[1.02] mb-1`}
                          >
                            <div className="flex items-center gap-1.5">
                              <div className={`h-1.5 w-1.5 shrink-0 rounded-full ${sc.dot}`} />
                              <p className={`text-[11px] font-medium truncate ${sc.text}`}>
                                {apt.customerName}
                              </p>
                            </div>
                            <p className="mt-0.5 text-[10px] text-white/30 truncate">
                              {format(parseISO(apt.startTime), "HH:mm")} · {apt.serviceName}
                            </p>
                          </button>
                        );
                      })}
          ...
          <div className="mx-4 w-full max-w-md animate-scale-in rounded-2xl border border-white/[0.06] bg-[#111] shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4 p-6">
              ...
              <div className="space-y-3 rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 text-sm">
                ...
                <div className="flex items-center gap-2"><span className="h-3.5 w-3.5 text-[10px] text-center text-white/30">🔧</span><span className="text-white/40">Servicio:</span><span>{selected.serviceName}</span></div>
                <div className="flex items-center gap-2"><span className="h-3.5 w-3.5 text-[10px] text-center text-white/30">👤</span><span className="text-white/40">Staff:</span><span>{selected.staffName}</span></div>
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {selected.status === "PENDING" && (
                      <>
                        <button onClick={() => handleStatus("CONFIRMED")} disabled={loading !== null}
                          className="flex items-center justify-center gap-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 py-2.5 text-sm font-medium text-emerald-400 transition-all hover:bg-emerald-500/20 disabled:opacity-50">
                          {loading === "CONFIRMED" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Check className="h-3.5 w-3.5" />} Confirmar
                        </button>
                        <button onClick={() => handleStatus("CANCELLED")} disabled={loading !== null}
                          className="flex items-center justify-center gap-1.5 rounded-xl bg-red-500/10 border border-red-500/20 py-2.5 text-sm font-medium text-red-400 transition-all hover:bg-red-500/20 disabled:opacity-50">
                          {loading === "CANCELLED" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <X className="h-3.5 w-3.5" />} Cancelar
                        </button>
                      </>
                    )}
                    {selected.status === "CONFIRMED" && (
                      <>
                        <button onClick={() => handleStatus("CHECKED_IN")} disabled={loading !== null}
                          className="flex items-center justify-center gap-1.5 rounded-xl bg-blue-500/10 border border-blue-500/20 py-2.5 text-sm font-medium text-blue-400 transition-all hover:bg-blue-500/20 disabled:opacity-50">
                          {loading === "CHECKED_IN" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <UserCheck className="h-3.5 w-3.5" />} Asistió
                        </button>
                        <button onClick={() => handleStatus("NO_SHOW")} disabled={loading !== null}
                          className="flex items-center justify-center gap-1.5 rounded-xl bg-amber-500/10 border border-amber-500/20 py-2.5 text-sm font-medium text-amber-400 transition-all hover:bg-amber-500/20 disabled:opacity-50">
                          {loading === "NO_SHOW" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <UserX className="h-3.5 w-3.5" />} No asistió
                        </button>
                        <button onClick={() => handleStatus("CANCELLED")} disabled={loading !== null}
                          className="col-span-2 flex items-center justify-center gap-1.5 rounded-xl bg-red-500/10 border border-red-500/20 py-2.5 text-sm font-medium text-red-400 transition-all hover:bg-red-500/20 disabled:opacity-50">
                          {loading === "CANCELLED" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <X className="h-3.5 w-3.5" />} Cancelar cita
                        </button>
                      </>
                    )}
                  </div>
