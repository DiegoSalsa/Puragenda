...
function LocaleFlag({
  ...
  if (locale === "es") {
    return (
      <svg viewBox="0 0 24 16" className={baseClass} aria-hidden="true">
        ...
        <path
          fill="#fff"
          d="m4 1.7.7 1.5 1.6.2-1.2 1.1.3 1.6L4 5.3 2.6 6l.3-1.5-1.2-1.1 1.6-.2z"
        />
      </svg>
    );
  }
    ...
    return (
      <svg viewBox="0 0 24 16" className={baseClass} aria-hidden="true">
        ...
        <g fill="#fff">
          ...
          <circle cx="3.5" cy="4" r=".6" />
          <circle cx="6.5" cy="4" r=".6" />
          <circle cx="2" cy="6" r=".6" />
  ...
  if (locale === "pt") {
    return (
      <svg viewBox="0 0 24 16" className={baseClass} aria-hidden="true">
        ...
        <path fill="#ffdf00" d="m12 2 9 6-9 6-9-6z" />
        <circle cx="12" cy="8" r="3.2" fill="#002776" />
        <path
          d="M9.4 7.2c1.7-.7 3.8-.5 5.4.5"
          fill="none"
          stroke="#fff"
          strokeWidth=".6"
        />
  ...
  if (locale === "de") {
    return (
      <svg viewBox="0 0 24 16" className={baseClass} aria-hidden="true">
        <path fill="#000" d="M0 0h24v5.34H0z" />
        <path fill="#dd0000" d="M0 5.33h24v5.34H0z" />
  ...
  return (
    <svg viewBox="0 0 24 16" className={baseClass} aria-hidden="true">
      ...
      <path
        fill="#ffde00"
        d="m4 2 .8 1.6 1.8.3-1.3 1.3.3 1.8L4 6.1 2.4 7l.3-1.8-1.3-1.3 1.8-.3z"
      />
      <g fill="#ffde00">
        <circle cx="8" cy="2" r=".55" />
        <circle cx="10" cy="4" r=".55" />
        <circle cx="10" cy="7" r=".55" />
        <circle cx="8" cy="9" r=".55" />
      </g>
    </svg>
  );
}

function localeFromCoordinates(
  latitude: number,
  longitude: number,
): AppLocale | null {
  if (
    ...
    return "es";
  if (latitude >= 18 && latitude <= 72 && longitude >= -170 && longitude <= -66)
    return "en";
  ...
  if (latitude >= 41 && latitude <= 52 && longitude >= -5 && longitude <= 10)
    return "fr";
  if (latitude >= 47 && latitude <= 55 && longitude >= 5 && longitude <= 16)
    return "de";
...
export function LanguageSwitcher({
    ...
    function positionMenu() {
      ...
      if (mobile) {
        setMenuPosition({
          ...
          maxHeight: Math.max(280, viewportHeight - 24),
          mobile: true,
        });
        return;
      }
      ...
      setMenuPosition({
        left,
        width,
        top: openAbove ? undefined : rect.bottom + 10,
        bottom: openAbove ? viewportHeight - rect.top + 10 : undefined,
        maxHeight: Math.max(240, (openAbove ? rect.top : spaceBelow) - 22),
        mobile: false,
      });
    }
  ...
  function detectFromLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        const coordinateLocale = localeFromCoordinates(
          coords.latitude,
          coords.longitude,
        );
  ...
  return (
    ...
    >
      <button
        ...
        className="group inline-flex min-h-10 max-w-full items-center justify-center gap-2 whitespace-nowrap rounded-full border border-border/70 bg-card/90 px-3 py-2 text-sm font-semibold text-foreground shadow-sm transition-all hover:-translate-y-0.5 hover:border-[#7C3AED]/40 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#7C3AED]/35 disabled:cursor-wait disabled:opacity-65"
      >
        <LocaleFlag locale={locale} />
        <span>{compact ? activeLocale.short : activeLocale.native}</span>
        {pending ? (
          <Loader2
            className="h-3.5 w-3.5 animate-spin text-[#7C3AED]"
            aria-hidden="true"
          />
        ) : (
          <ChevronDown
          ...
          <>
            {menuPosition.mobile && (
              <button
                ...
                className="fixed inset-0 z-[19990] cursor-default bg-black/45 backdrop-blur-[2px]"
              />
            )}
            <div
              ref={menuRef}
              ...
              style={
                menuPosition.mobile
                  ? {
                      ...
                      maxHeight:
                        "calc(100dvh - 1.5rem - env(safe-area-inset-bottom))",
                    }
                  : {
                      left: menuPosition.left,
              ...
              className={`fixed z-[20000] overflow-y-auto overscroll-contain border border-border bg-background p-2 shadow-[0_18px_55px_rgba(0,0,0,0.24)] ${menuPosition.mobile ? "rounded-3xl" : "rounded-2xl"}`}
            >
              ...
              <div className="mt-2 border-t border-border pt-2">
                ...
                >
                  ...
                  <span className="min-w-0">
                    ...
                    <span
                      className={`mt-0.5 block text-xs leading-relaxed ${locationState === "error" ? "text-red-500" : "text-muted-foreground"}`}
                    >
                      {locationState === "error"
                        ? t("locationError")
                        : t("locationPrivacy")}
