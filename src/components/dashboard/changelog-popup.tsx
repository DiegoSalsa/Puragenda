          ...
          <div className="min-h-0 overflow-y-auto overscroll-contain p-4 sm:p-5 [@media(max-height:620px)]:p-3">
            ...
            <div className="mb-4 space-y-2.5 [@media(max-height:620px)]:mb-3 [@media(max-height:620px)]:space-y-2">
              {latestUpdate.features.slice(0, 3).map((feature, index) => (
                <div
                  key={index}
                  className="flex items-start gap-3 rounded-xl border-2 border-black bg-[#BFFCC6] p-2.5 shadow-[2px_2px_0_#000] [@media(max-height:620px)]:p-2"
                >
                  <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-black" strokeWidth={3} />
                  <span className="text-sm font-bold leading-snug text-black [@media(max-height:620px)]:text-[13px] [@media(max-height:620px)]:leading-[1.25]">
                    {feature}
                  </span>
                </div>
              ))}
