...
export function LoyaltyCard({ businessName, logoUrl, currentStamps, stampsRequired, rewardLabel, enabled = true, compact = false }: {
  ...
  const t = useTranslations("loyalty.card");
  ...
  const progress = current >= required ? t("progress.unlocked")
    : current === 0 ? t("progress.first")
      : remaining === 1 ? t("progress.one")
        : current >= Math.ceil(required / 2) ? t("progress.halfway", { count: remaining })
    ...
    <article className={`rounded-[1.75rem] border-4 border-black bg-[#ffb5e8] ${compact ? "p-4" : "p-5 sm:p-7"} shadow-[7px_7px_0_#000]`}>
      <div className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0"><p className="truncate text-xs font-black uppercase tracking-[0.13em]">{businessName}</p><p className="text-sm font-black">{t("title")}</p></div>
        </div>
      ...
      <div className="mt-5 rounded-2xl border-2 border-black bg-[#fffaf0] p-4">
        <p className="font-black">{enabled ? progress : t("programDisabled")}</p>
        <p className="mt-2 text-[10px] font-black uppercase tracking-[0.13em] text-black/55">{t("completionLabel")}</p>
