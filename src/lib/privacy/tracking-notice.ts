...
const notices: Record<string, { tracking: TrackingNotice; terms: TermsNotice }> = {
  es: {
    tracking: {
      ...
      providers: "Los eventos se guardan en nuestra base operativa de Supabase y se sirven mediante Vercel. PostHog solo se activa si está configurado y después de tu consentimiento; la captura de sesiones está desactivada por defecto. Estos proveedores pueden procesar datos fuera de Chile bajo contratos y garantías de protección aplicables.",
  ...
  en: {
    tracking: {
      ...
      providers: "Events are stored in our operational Supabase database and served through Vercel. PostHog is activated only when configured and after consent; session capture is disabled by default. These providers may process data outside Chile under applicable contracts and safeguards.",
  ...
  it: {
    tracking: {
      ...
      providers: "Gli eventi sono salvati nel database operativo Supabase e serviti tramite Vercel. PostHog viene attivato solo se configurato e dopo il consenso; la registrazione delle sessioni è disattivata per impostazione predefinita. Questi fornitori possono trattare dati fuori dal Cile in base a contratti e garanzie applicabili.",
  ...
  pt: {
    tracking: {
      ...
      providers: "Os eventos são armazenados no banco operacional Supabase e servidos pela Vercel. O PostHog só é ativado quando configurado e após o consentimento; a gravação de sessões fica desativada por padrão. Esses fornecedores podem processar dados fora do Chile conforme contratos e salvaguardas aplicáveis.",
  ...
  fr: {
    tracking: {
      ...
      providers: "Les événements sont stockés dans notre base opérationnelle Supabase et servis via Vercel. PostHog n'est activé que s'il est configuré et après consentement ; l'enregistrement des sessions est désactivé par défaut. Ces fournisseurs peuvent traiter des données hors du Chili en vertu de contrats et garanties applicables.",
  ...
  de: {
    tracking: {
      ...
      providers: "Ereignisse werden in unserer operativen Supabase-Datenbank gespeichert und über Vercel bereitgestellt. PostHog wird nur aktiviert, wenn es konfiguriert ist und nach Einwilligung; die Sitzungsaufzeichnung ist standardmäßig deaktiviert. Diese Anbieter können Daten außerhalb Chiles gemäß geltenden Verträgen und Schutzmaßnahmen verarbeiten.",
  ...
  "zh-CN": {
    tracking: {
      ...
      providers: "事件存储在我们的 Supabase 运营数据库中，并通过 Vercel 提供。只有在配置了 PostHog 且获得同意后才会启用；默认关闭会话录制。这些供应商可能依据适用合同和保障措施在智利境外处理数据。",
