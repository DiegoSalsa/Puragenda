...
export default async function ClientPortalPage({
  ...
  const giftCardT = await getTranslations("giftCardsPortal");
  ...
  const [data, profile, giftCards] = await Promise.all([
    getClientPortalData(email),
    getClientPortalProfile(email),
    account ? prisma.giftCard.findMany({
      where: { claimedByAccountId: account.id },
      include: { business: { select: { name: true, slug: true, logoUrl: true } }, entitlements: true, transactions: { orderBy: { createdAt: "desc" }, take: 5 } },
      ...
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 sm:py-12">
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[1.45fr_0.8fr]">
          <div className="space-y-10">
            <section id="mis-gift-cards">
              <div className="mb-4 flex flex-wrap items-center gap-3">
                <Gift className="h-6 w-6" strokeWidth={2.8} />
                <h2 className="text-xl font-black sm:text-2xl">{giftCardT("myCards")}</h2>
                <span className="rounded-full border-2 border-black bg-[#ffb5e8] px-2.5 py-0.5 text-xs font-black">{giftCards.filter((card) => card.status === "ACTIVE").length}</span>
                {account && <Link href="/mi-agenda/gift-cards/reclamar" className="ml-auto rounded-xl border-2 border-black bg-white px-3 py-2 text-xs font-black shadow-[2px_2px_0_#000]">{giftCardT("addCode")}</Link>}
              </div>
              {giftCards.length === 0 ? <div className="rounded-2xl border-3 border-dashed border-black/35 bg-white/60 p-7 text-center"><Gift className="mx-auto h-9 w-9 text-black/35" /><p className="mt-3 font-black">{giftCardT("emptyTitle")}</p><p className="mt-1 text-sm font-medium text-black/55">{giftCardT("emptyDescription")}</p></div> : <div className="grid gap-4 sm:grid-cols-2">{giftCards.map((card) => {
                const used = card.type === "BALANCE" ? (card.initialBalance || 0) - (card.remainingBalance || 0) : card.entitlements.reduce((sum, item) => sum + item.quantityInitial - item.quantityRemaining, 0);
                const totalBenefits = card.entitlements.reduce((sum, item) => sum + item.quantityInitial, 0);
                const label = card.status === "VOIDED" ? giftCardT("voided") : card.status === "DEPLETED" ? giftCardT("depleted") : used > 0 ? giftCardT("partiallyUsed") : giftCardT("available");
                return <article key={card.id} className="overflow-hidden rounded-2xl border-3 border-black bg-white shadow-[5px_5px_0_#000]"><div className="p-5" style={{ background: card.backgroundColorSnapshot, color: card.textColorSnapshot }}><div className="flex items-center justify-between gap-3"><p className="text-xs font-black uppercase tracking-wider">{card.business.name}</p><span className="rounded-full border-2 border-black bg-white px-2 py-0.5 text-[10px] font-black text-black">{label}</span></div><h3 className="mt-3 text-xl font-black">{card.nameSnapshot}</h3><p className="mt-1 font-mono text-xs font-black">{card.publicCode}</p></div><div className="p-5">{card.type === "BALANCE" ? <div className="grid grid-cols-2 gap-3"><div><p className="text-[10px] font-black uppercase text-black/45">{giftCardT("originalValue")}</p><p className="font-black">{formatPrice(card.initialBalance || 0, card.currencyCode)}</p></div><div><p className="text-[10px] font-black uppercase text-black/45">{giftCardT("availableBalance")}</p><p className="text-lg font-black">{formatPrice(card.remainingBalance || 0, card.currencyCode)}</p></div><p className="col-span-2 text-xs font-bold text-black/55">{giftCardT("usedAmount", { amount: formatPrice(used, card.currencyCode) })}</p></div> : <><ul className="space-y-2">{card.entitlements.map((item) => <li key={item.id} className="flex items-center gap-2 text-sm font-bold"><span>{item.quantityRemaining > 0 ? "✓" : "○"}</span><span className="flex-1">{item.serviceNameSnapshot}</span><span>{giftCardT("unitCount", { remaining: item.quantityRemaining, initial: item.quantityInitial })}</span></li>)}</ul><p className="mt-3 text-xs font-bold text-black/55">{giftCardT("benefitsUsed", { used, total: totalBenefits })}</p></>} {card.status === "ACTIVE" && <Link href={`/widget/${card.business.slug}`} className="mt-4 block rounded-xl border-2 border-black bg-[#fff5ba] px-4 py-2 text-center text-sm font-black shadow-[3px_3px_0_#000]">{giftCardT("reserve")}</Link>}</div></article>;
              })}</div>}
            </section>
