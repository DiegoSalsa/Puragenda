          ...
          <div className="space-y-10">
            <section id="mis-gift-cards">
              <div className="mb-4 flex flex-wrap items-center gap-3">
                ...
                <h2 className="text-xl font-black sm:text-2xl">Mis Gift Cards</h2>
                ...
                {account && <Link href="/mi-agenda/gift-cards/reclamar" className="ml-auto rounded-xl border-2 border-black bg-white px-3 py-2 text-xs font-black shadow-[2px_2px_0_#000]">Agregar con código</Link>}
              </div>
              {giftCards.length === 0 ? <div className="rounded-2xl border-3 border-dashed border-black/35 bg-white/60 p-7 text-center"><Gift className="mx-auto h-9 w-9 text-black/35" /><p className="mt-3 font-black">Aún no agregas Gift Cards</p><p className="mt-1 text-sm font-medium text-black/55">Cuando recibas una, agrégala con su enlace o código.</p></div> : <div className="grid gap-4 sm:grid-cols-2">{giftCards.map((card) => {
                ...
                const label = card.status === "VOIDED" ? "Anulada" : card.status === "DEPLETED" ? "Agotada" : used > 0 ? "Parcialmente utilizada" : "Disponible";
                return <article key={card.id} className="overflow-hidden rounded-2xl border-3 border-black bg-white shadow-[5px_5px_0_#000]"><div className="p-5" style={{ background: card.backgroundColorSnapshot, color: card.textColorSnapshot }}><div className="flex items-center justify-between gap-3"><p className="text-xs font-black uppercase tracking-wider">{card.business.name}</p><span className="rounded-full border-2 border-black bg-white px-2 py-0.5 text-[10px] font-black text-black">{label}</span></div><h3 className="mt-3 text-xl font-black">{card.nameSnapshot}</h3><p className="mt-1 font-mono text-xs font-black">{card.publicCode}</p></div><div className="p-5">{card.type === "BALANCE" ? <div className="grid grid-cols-2 gap-3"><div><p className="text-[10px] font-black uppercase text-black/45">Valor original</p><p className="font-black">{formatPrice(card.initialBalance || 0, card.currencyCode)}</p></div><div><p className="text-[10px] font-black uppercase text-black/45">Saldo disponible</p><p className="text-lg font-black">{formatPrice(card.remainingBalance || 0, card.currencyCode)}</p></div><p className="col-span-2 text-xs font-bold text-black/55">Utilizado: {formatPrice(used, card.currencyCode)}</p></div> : <><ul className="space-y-2">{card.entitlements.map((item) => <li key={item.id} className="flex items-center gap-2 text-sm font-bold"><span>{item.quantityRemaining > 0 ? "✓" : "○"}</span><span className="flex-1">{item.serviceNameSnapshot}</span><span>{item.quantityRemaining} de {item.quantityInitial}</span></li>)}</ul><p className="mt-3 text-xs font-bold text-black/55">{used} de {totalBenefits} beneficios utilizados</p></>} {card.status === "ACTIVE" && <Link href={`/widget/${card.business.slug}`} className="mt-4 block rounded-xl border-2 border-black bg-[#fff5ba] px-4 py-2 text-center text-sm font-black shadow-[3px_3px_0_#000]">Reservar</Link>}</div></article>;
              })}</div>}
            </section>
