      ...
      {locations.map((location) => <div key={location.id} className="rounded-xl border border-border bg-muted/30 p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <div className="flex items-center gap-2"><Building2 className="h-4 w-4 text-brand-foreground" /><p className="font-semibold">{location.name}</p>{location.isPrimary && <span className="rounded-full bg-[#7C3AED]/10 px-2 py-0.5 text-[10px] font-bold text-brand-foreground"><LocalizedText id="Qh2uO8AkshAG" /></span>}</div>
