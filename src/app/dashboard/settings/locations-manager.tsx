...
export function LocationsManager({ locations, defaultTimezone, countryCode, services }: { locations: Location[]; defaultTimezone: string; countryCode: string; services: { id: string; name: string }[] }) {
  ...
  return <div className="space-y-4">
    <p className="text-sm text-muted-foreground">Todos los planes pueden usar varias sucursales. Cada una tendrá agenda, horarios y servicios disponibles propios.</p>
      ...
      {locations.map((location) => <div key={location.id} className="rounded-xl border border-border bg-muted/30 p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <div className="flex items-center gap-2"><Building2 className="h-4 w-4 text-[#7C3AED]" /><p className="font-semibold">{location.name}</p>{location.isPrimary && <span className="rounded-full bg-[#7C3AED]/10 px-2 py-0.5 text-[10px] font-bold text-[#7C3AED]">PRINCIPAL</span>}</div>
            <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground"><MapPin className="h-3 w-3" />{location.address || "Dirección pendiente"} · {location.timezone}</p>
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" disabled={busy === `edit:${location.id}`} onClick={() => openEdit(location)} className="flex items-center gap-1 rounded-lg border border-border px-3 py-2 text-xs font-semibold disabled:opacity-50"><Pencil className="h-3.5 w-3.5" />Editar</button>
            ...
            <button type="button" disabled={location.isPrimary || busy === `delete:${location.id}`} onClick={() => remove(location)} className="flex items-center gap-1 rounded-lg border border-red-500/30 px-3 py-2 text-xs font-semibold text-red-500 disabled:opacity-50"><Trash2 className="h-3.5 w-3.5" />Eliminar</button>
          </div>
        </div>
        {editing?.id === location.id && <div className="mt-4 grid gap-3 border-t border-border pt-4 md:grid-cols-2">
          <input value={editDraft.name} onChange={(event) => setEditDraft({ ...editDraft, name: event.target.value })} placeholder="Nombre de la sucursal" className="rounded-lg border border-border bg-background px-3 py-2 text-sm" />
          <input value={editDraft.address} onChange={(event) => setEditDraft({ ...editDraft, address: event.target.value })} placeholder="Dirección" className="rounded-lg border border-border bg-background px-3 py-2 text-sm" />
          <input value={editDraft.mapsUrl} onChange={(event) => setEditDraft({ ...editDraft, mapsUrl: event.target.value })} placeholder="Enlace de Google Maps (opcional)" className="rounded-lg border border-border bg-background px-3 py-2 text-sm" />
          <TimezoneSelector value={editDraft.timezone} countryCode={countryCode} onValueChange={(timezone) => setEditDraft({ ...editDraft, timezone })} />
          <div className="flex gap-2 md:col-span-2"><button type="button" onClick={saveEdit} disabled={busy === `edit:${location.id}`} className="flex items-center gap-2 rounded-lg bg-[#7C3AED] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{busy === `edit:${location.id}` ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Guardar cambios</button><button type="button" onClick={() => setEditing(null)} className="rounded-lg border border-border px-4 py-2 text-sm">Cancelar</button></div>
        </div>}
      </div>)}
    </div>
    {creating ? <div className="grid gap-3 rounded-xl border border-[#7C3AED]/30 bg-[#7C3AED]/5 p-4 md:grid-cols-2">
      <input value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} placeholder="Nombre de la sucursal" className="rounded-lg border border-border bg-background px-3 py-2 text-sm" />
      <input value={draft.address} onChange={(event) => setDraft({ ...draft, address: event.target.value })} placeholder="Dirección" className="rounded-lg border border-border bg-background px-3 py-2 text-sm" />
      <input value={draft.mapsUrl} onChange={(event) => setDraft({ ...draft, mapsUrl: event.target.value })} placeholder="Enlace de Google Maps (opcional)" className="rounded-lg border border-border bg-background px-3 py-2 text-sm" />
      <TimezoneSelector value={draft.timezone} countryCode={countryCode} onValueChange={(timezone) => setDraft({ ...draft, timezone })} />
      <div className="flex gap-2 md:col-span-2"><button type="button" onClick={create} disabled={busy === "new"} className="flex items-center gap-2 rounded-lg bg-[#7C3AED] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">{busy === "new" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Guardar sucursal</button><button type="button" onClick={() => setCreating(false)} className="rounded-lg border border-border px-4 py-2 text-sm">Cancelar</button></div>
    </div> : <button type="button" onClick={() => setCreating(true)} className="flex items-center gap-2 rounded-xl border border-[#7C3AED]/30 bg-[#7C3AED]/10 px-4 py-2.5 text-sm font-semibold text-[#7C3AED]"><Plus className="h-4 w-4" />Agregar sucursal</button>}
