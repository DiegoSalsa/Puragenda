...
export function LoyaltyConfigForm({ initialData, business, services, clients }: Props) {
  ...
  const presetGoals = [5, 8, 10, 12];
  const [customGoal, setCustomGoal] = useState(!presetGoals.includes(initialData.stampsRequired));
  const [previewState, setPreviewState] = useState<"start" | "progress" | "oneLeft" | "reward">("progress");
  const [clientSearch, setClientSearch] = useState("");
  const filteredClients = useMemo(() => {
    const query = clientSearch.trim().toLocaleLowerCase();
    if (!query) return clients;
    return clients.filter((client) => `${client.name} ${client.email}`.toLocaleLowerCase().includes(query));
  ...
  const previewStamps = loyaltyPreviewStamps(form.stampsRequired, previewState);
  const freeServiceMissing = form.rewardType === "FREE_SERVICE" && !form.rewardServiceId;

  async function save(event: React.FormEvent) {
    event.preventDefault();
    if (freeServiceMissing) { setMessage({ ok: false, text: t("missingFreeService") }); return; }
    ...
    <form onSubmit={save} className="grid items-start gap-7 xl:grid-cols-[minmax(0,1.15fr)_minmax(340px,.85fr)]">
      <div className="space-y-5">
        <section className="rounded-2xl border-3 border-black bg-white p-5 shadow-[4px_4px_0_#000]">
          <div className="flex items-start justify-between gap-4"><div><p className="font-black">{t("program")}</p><p className="mt-1 text-sm font-medium text-black/55">{t("subtitle")}</p></div><button type="button" role="switch" aria-label={t("program")} aria-checked={form.isLoyaltyEnabled} onClick={() => setForm((current) => ({ ...current, isLoyaltyEnabled: !current.isLoyaltyEnabled }))} className={`relative h-8 w-14 shrink-0 rounded-full border-3 border-black p-0.5 transition-colors focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#c4b5fd] ${form.isLoyaltyEnabled ? "bg-[#bffcc6]" : "bg-black/15"}`}><span className={`block h-5 w-5 rounded-full border-2 border-black bg-white shadow-[1px_1px_0_#000] transition-transform duration-200 ${form.isLoyaltyEnabled ? "translate-x-5" : "translate-x-0"}`} /></button></div>
        </section>

        <fieldset disabled={!form.isLoyaltyEnabled} className="space-y-5 disabled:opacity-50">
          <section className="rounded-2xl border-3 border-black bg-white p-5 shadow-[4px_4px_0_#000]"><legend className="px-1 text-lg font-black">{t("goal")}</legend><p className="mb-4 text-sm font-semibold text-black/60">{t("goalQuestion")}</p><div className="grid grid-cols-5 gap-2">{presetGoals.map((value) => <button key={value} type="button" onClick={() => { setCustomGoal(false); setForm((current) => ({ ...current, stampsRequired: value })); }} className={`rounded-xl border-2 border-black py-2 font-black ${!customGoal && form.stampsRequired === value ? "bg-[#c4b5fd] shadow-[2px_2px_0_#000]" : "bg-white"}`}>{value}</button>)}<button type="button" onClick={() => setCustomGoal(true)} aria-pressed={customGoal} className={`rounded-xl border-2 border-black py-2 font-black ${customGoal ? "bg-[#c4b5fd] shadow-[2px_2px_0_#000]" : "bg-white"}`}>{t("other")}</button></div>{customGoal && <div className="mt-4"><label className="text-sm font-black" htmlFor="custom-stamps">{t("numberOfVisits")}</label><input id="custom-stamps" type="number" min={2} max={30} value={form.stampsRequired} onChange={(event) => { setCustomGoal(true); setForm((current) => ({ ...current, stampsRequired: Number(event.target.value) })); }} className="mt-1 h-11 w-full rounded-xl border-2 border-black px-3 font-black outline-none focus:ring-4 focus:ring-[#c4b5fd]" /></div>}<p className="mt-3 text-sm font-black">{t("goalSummary", { count: form.stampsRequired })}</p></section>

          <section className="rounded-2xl border-3 border-black bg-[#fff5ba] px-4 py-3 shadow-[4px_4px_0_#000]"><h2 className="font-black">{t("stampRuleTitle")}</h2><p className="mt-1 text-sm font-semibold">✓ {t("stampRule")}</p></section>
          ...
          <section className="rounded-2xl border-3 border-black bg-white p-5 shadow-[4px_4px_0_#000]"><h2 className="text-lg font-black">{t("rewardTitle")}</h2><div className="mt-4 grid gap-3 sm:grid-cols-2">{rewardOptions.map(({ type, icon: Icon, color }) => <button key={type} type="button" onClick={() => setForm((current) => ({ ...current, rewardType: type }))} aria-pressed={form.rewardType === type} className={`${color} flex items-center gap-3 rounded-2xl border-3 border-black p-4 text-left font-black ${form.rewardType === type ? "shadow-[4px_4px_0_#000] -translate-y-0.5" : "opacity-70"}`}><Icon className="h-5 w-5" />{t(`rewardTypes.${type}`)}</button>)}</div>
            <div className="mt-5 space-y-4">
              ...
              {form.rewardType === "FREE_SERVICE" && <div><label htmlFor="reward-service" className="text-sm font-black">{t("freeService")}</label><select id="reward-service" value={form.rewardServiceId ?? ""} onChange={(event) => setForm((current) => ({ ...current, rewardServiceId: event.target.value || null }))} aria-invalid={freeServiceMissing} className={`mt-1 h-11 w-full rounded-xl border-2 bg-[#fffaf0] px-3 font-bold ${freeServiceMissing ? "border-red-600" : "border-black"}`}><option value="">{t("selectService")}</option>{services.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</select>{freeServiceMissing && <p role="alert" className="mt-2 text-sm font-black text-red-700">{t("missingFreeService")}</p>}<p className="mt-2 text-xs font-semibold text-black/55">{t("freeServiceHint")}</p></div>}
        ...
        <div className="flex items-center gap-4"><button type="submit" disabled={saving || freeServiceMissing} className="inline-flex h-12 items-center gap-2 rounded-xl border-3 border-black bg-[#bffcc6] px-6 font-black shadow-[4px_4px_0_#000] disabled:opacity-50">{saving ? <Loader2 className="h-5 w-5 animate-spin" /> : <Save className="h-5 w-5" />} {t("save")}</button>{message && <p role="status" className={`text-sm font-black ${message.ok ? "text-emerald-700" : "text-red-700"}`}>{message.text}</p>}</div>
      </div>

      <aside className="xl:sticky xl:top-6"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><p className="text-sm font-black uppercase tracking-[0.13em]">{t("preview")}</p><div className="flex flex-wrap gap-1" aria-label={t("preview")} role="group">{(["start", "progress", "oneLeft", "reward"] as const).map((state) => <button key={state} type="button" onClick={() => setPreviewState(state)} aria-pressed={previewState === state} className={`rounded-lg border-2 border-black px-2 py-1 text-[10px] font-black ${previewState === state ? "bg-[#c4b5fd] shadow-[2px_2px_0_#000]" : "bg-white"}`}>{t(`previewStates.${state}`)}</button>)}</div></div><LoyaltyCard businessName={business.name} logoUrl={business.logoUrl} currentStamps={previewStamps} stampsRequired={form.stampsRequired} rewardLabel={rewardLabel} enabled={form.isLoyaltyEnabled} /></aside>
    </form>

    <section className="overflow-hidden rounded-2xl border-3 border-black bg-white shadow-[5px_5px_0_#000]"><div className="bg-[#c4b5fd] p-5"><h2 className="text-lg font-black">{t("adjustments")}</h2><p className="text-sm font-semibold">{t("adjustmentHint")}</p>{clients.length > 8 && <label className="mt-3 block"><span className="sr-only">{t("searchClients")}</span><input value={clientSearch} onChange={(event) => setClientSearch(event.target.value)} placeholder={t("searchClients")} className="h-10 w-full rounded-xl border-2 border-black bg-white px-3 text-sm font-semibold outline-none focus:ring-4 focus:ring-[#fff5ba]" /></label>}</div>{filteredClients.length ? filteredClients.map((client) => <ManualAdjustmentRow key={client.id} client={client} required={form.stampsRequired} />) : <p className="p-5 text-sm font-semibold">{clients.length ? t("noMatchingClients") : t("noClients")}</p>}</section>
  </div>;
}
