...
export async function uploadWidgetAsset(input: {
  ...
  const metadata = inspectImage(bytes, input.file.type);
  ...
  if (process.env.NODE_ENV === "production") {
    ...
    const base64 = `data:${metadata.mimeType};base64,${bytes.toString("base64")}`;
    ...
    provider = "local";
    publicId = `${input.businessId}/${assetToken}.${metadata.extension}`;
    ...
    await writeFile(path.join(directory, `${assetToken}.${metadata.extension}`), bytes, {
      flag: "wx",
    });
