...
async function main() {
  ...
  // ─── Biz 1: PuroCode Demo (PRO, paid) ───
  console.log("🏢 PuroCode Demo (Pro)...");
  const biz1 = await prisma.business.create({
    data: { name: "PuroCode Demo", slug: "purocode-demo", apiKey: apiKey(), ownerId: admin.id, primaryColor: "#7C3AED", secondaryColor: "#5B21B6" },
  });
  await prisma.subscription.create({
    data: { businessId: biz1.id, plan: "PRO", status: "ACTIVE", billingCycle: "MONTHLY" },
  });
  // Business hours (Mon-Fri 9-19, Sat 10-14)
  for (let d = 0; d <= 6; d++) {
    await prisma.businessHours.create({
      data: { businessId: biz1.id, dayOfWeek: d, startTime: d === 0 ? "00:00" : d === 6 ? "10:00" : "09:00", endTime: d === 0 ? "00:00" : d === 6 ? "14:00" : "19:00", isOpen: d >= 1 && d <= 6 },
    });
  }
  ...
  const biz2 = await prisma.business.create({
    data: { name: "Estética Bella", slug: "estetica-bella", apiKey: apiKey(), ownerId: u2.id, primaryColor: "#E91E8C", secondaryColor: "#C2185B", brandColor: "E91E8C" },
  });
  await prisma.subscription.create({
    data: { businessId: biz2.id, plan: "BASIC", status: "TRIALING", billingCycle: "MONTHLY", isTrial: true, trialEndsAt: addDays(now, 25), extraStaffCount: 1 },
  });
  await prisma.blacklistedIp.create({ data: { ip: "192.168.1.50", reason: "Trial usado", userId: u2.id } });
  for (let d = 0; d <= 6; d++) {
    await prisma.businessHours.create({
  ...
  // ─── Biz 3: Barbería (INDIVIDUAL, paid) ───
  ...
  const biz3 = await prisma.business.create({
    data: { name: "Barbería Santiago", slug: "barberia-stgo", apiKey: apiKey(), ownerId: u3.id, primaryColor: "#D97706", secondaryColor: "#92400E" },
  });
  await prisma.subscription.create({
    data: { businessId: biz3.id, plan: "INDIVIDUAL", status: "ACTIVE", billingCycle: "ANNUAL" },
  });
  for (let d = 0; d <= 6; d++) {
    await prisma.businessHours.create({
      data: { businessId: biz3.id, dayOfWeek: d, startTime: "09:00", endTime: "18:00", isOpen: d >= 1 && d <= 5 },
    });
  }
  await prisma.staff.create({ data: { name: "Carlos Ruiz", email: "carlos@barberia.cl", businessId: biz3.id, userId: u3.id } });
  await prisma.service.create({ data: { name: "Corte Clásico", duration: 30, price: 8000, businessId: biz3.id } });
  // Blocked date
  const blockedDay = addDays(now, 3);
  ...
  const apts = [
    ...
    { customerName: "Ana Torres", customerEmail: "ana@email.com", startTime: setMinutes(setHours(t1, 14), 0), endTime: setMinutes(setHours(t1, 14), 45), businessId: biz1.id, serviceId: svc1.id, staffId: s1b.id, status: "CHECKED_IN" as const },
    { customerName: "Sofía Herrera", customerEmail: "sofia@email.com", startTime: setMinutes(setHours(t1, 9), 0), endTime: setMinutes(setHours(t1, 9), 30), businessId: biz2.id, serviceId: svc3.id, staffId: s2a.id, status: "CONFIRMED" as const },
    { customerName: "Isabella Díaz", customerEmail: "isabella@email.com", startTime: setMinutes(setHours(t2, 10), 0), endTime: setMinutes(setHours(t2, 11), 0), businessId: biz2.id, serviceId: svc4.id, staffId: s2b.id, status: "NO_SHOW" as const },
    { customerName: "TEST CANCELADA", customerEmail: "test@email.com", startTime: setMinutes(setHours(t1, 16), 0), endTime: setMinutes(setHours(t1, 17), 0), businessId: biz1.id, serviceId: svc1.id, staffId: s1a.id, status: "CANCELLED" as const },
  ];
  for (const a of apts) await prisma.appointment.create({ data: a });

  console.log("\n✅ Seed completado!");
  console.log("═══════════════════════════════════════");
  console.log(`  👑 admin@purocode.cl / purocode123`);
  console.log(`  👤 vale@esteticabella.cl / purocode123`);
  console.log(`  👤 carlos@barberia.cl / purocode123`);
  console.log("═══════════════════════════════════════\n");
}
